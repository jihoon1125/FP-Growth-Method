...
private:
	...
	ofstream flog;
public:
	FPGrowth(ofstream *fout, int threshold = 3) {//default threshold = 3
		this->threshold = threshold;
		flog.open("result.txt", ios::trunc);
		flog.setf(ios::fixed);
	...
	void FrequentPatternSetting() {
		...
		frequentPatterns = getFrequentPatterns(table, fpTree);	
	}

	map<set<string>, int> getFrequentPatterns(HeaderTable* pTable, FPNode* pTree);
	void powerSet(map<set<string>, int>* FrequentPattern, vector<string> data, string item, int frequency, int* flag, int depth);//making subset
